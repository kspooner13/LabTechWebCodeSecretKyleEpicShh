...
function LoadCalendarScript(callback) {
    function LoadFullCalendarScript() {
        if (!$.fn.fullCalendar) $.getScript(
            'plugins/fullcalendar/fullcalendar.js', callback);
        else if (callback && typeof callback === "function") callback();
    }
    if (!$.fn.moment) $.getScript('plugins/moment/moment.min.js',
        LoadFullCalendarScript);
    else LoadFullCalendarScript();
}

function LoadOpenLayersScript(callback) {
    if (!$.fn.OpenLayers) $.getScript(
        'http://www.openlayers.org/api/OpenLayers.js', callback);
    else if (callback && typeof callback === "function") callback();
}

function LoadTimePickerScript(callback) {
    if (!$.fn.timepicker) $.getScript(
...
function LoadBootstrapValidatorScript(callback) {
    if (!$.fn.bootstrapValidator) $.getScript(
...
function LoadSelect2Script(callback) {
    if (!$.fn.select2) $.getScript('plugins/select2/select2.min.js',
        callback);
    else if (callback && typeof callback === "function") callback();
}
...
function LoadDataTablesScripts(callback) {
    function LoadDatatables() {
        $.getScript('plugins/datatables/jquery.dataTables.js', function() {
            $.getScript('plugins/datatables/ZeroClipboard.js',
                function() {
                    $.getScript(
                        'plugins/datatables/TableTools.js',
                        function() {
                            $.getScript(
    ...
    if (!$.fn.dataTables) LoadDatatables();
    else if (callback && typeof callback === "function") callback();
}

function LoadFineUploader(callback) {
    if (!$.fn.fineuploader) $.getScript(
...
function LoadXChartScript(callback) {
    ...
    function LoadD3Script() {
        if (!$.fn.d3) $.getScript('plugins/d3/d3.v3.min.js', LoadXChart);
        else LoadXChart();
    }
    if (!$.fn.xcharts) LoadD3Script();
    else if (callback && typeof callback === "function") callback();
}
...
function LoadFlotScripts(callback) {
    ...
    function LoadFlotResizeScript() {
        $.getScript('plugins/flot/jquery.flot.resize.js',
            LoadFlotTimeScript);
    }
    ...
    if (!$.fn.flot) LoadFlotScript();
    else if (callback && typeof callback === "function") callback();
}

function LoadMorrisScripts(callback) {
    function LoadMorrisScript() {
        if (!$.fn.Morris) $.getScript('plugins/morris/morris.min.js',
            callback);
        else if (callback && typeof callback === "function") callback();
    }
    if (!$.fn.raphael) $.getScript('plugins/raphael/raphael-min.js',
        LoadMorrisScript);
    else LoadMorrisScript();
}

function LoadFancyboxScript(callback) {
    if (!$.fn.fancybox) $.getScript('plugins/fancybox/jquery.fancybox.js',
        callback);
    else if (callback && typeof callback === "function") callback();
}

function LoadKnobScripts(callback) {
    if (!$.fn.knob) $.getScript('plugins/jQuery-Knob/jquery.knob.js',
        callback);
    else if (callback && typeof callback === "function") callback();
}

function LoadSparkLineScript(callback) {
    if (!$.fn.sparkline) $.getScript(
        'plugins/sparkline/jquery.sparkline.min.js', callback);
    else if (callback && typeof callback === "function") callback();
}
...
function LoadAjaxContent(url) {
    $('.preloader').show();
    $.ajax({
        mimeType: 'text/php; charset=utf-8',
        url: url,
        type: 'GET',
...
function WinMove() {
    $("div.box").not('.no-drop').draggable({
        ...
        drop: function(event, ui) {
            ...
            setTimeout(function() {
                var dropmap = droppable.find(
                    '[id^=map-]');
                var dragmap = draggable.find(
                    '[id^=map-]');
                if (dragmap.length > 0 || dropmap.length >
                    0) {
                    dragmap.resize();
                    dropmap.resize();
                } else {
...
function ScreenSaver() {
    ...
    function draw() {
        ...
        for (var i = 0; i < particles.length; i++) {
            ...
            p.location.x = p.location.x + p.speed * Math.cos(p.angle *
                Math.PI / 180);
            p.location.y = p.location.y + p.speed * Math.sin(p.angle *
                Math.PI / 180);
    ...
    elem.knob({
        ...
        draw: function() {
            if (this.$.data('skin') == 'tron') {
                ...
                this.o.cursor && (sat = eat - 0.3) && (eat =
                    eat + 0.3);
                if (this.o.displayPrevious) {
                    ea = this.startAngle + this.angle(this.v);
                    this.o.cursor && (sa = ea - 0.3) && (ea =
                        ea + 0.3);
                    this.g.beginPath();
                ...
                this.g.arc(this.xy, this.xy, this.radius - this
                    .lineWidth, sat, eat, false);
                this.g.stroke();
                this.g.lineWidth = 2;
                this.g.beginPath();
                this.g.strokeStyle = this.o.fgColor;
                this.g.arc(this.xy, this.xy, this.radius - this
                    .lineWidth + 1 + this.lineWidth * 2 / 3,
                    0, 2 * Math.PI, false);
                this.g.stroke();
                return false;
            }
...
function drawMap(lon, lat, elem, layers) {
    ...
    var guestMarker = new OpenLayers.Feature.Vector(new OpenLayers.Geometry
        .Point(lon, lat).transform(epsg4326, projectTo));
    layerGuest.addFeatures(guestMarker);
    LayersArray.push(layerGuest);
    map.addLayers(LayersArray);
    if (layers.length > 1) map.addControl(new OpenLayers.Control.LayerSwitcher({
        'ascending': true
    }));
...
function DashboardTabChecker() {
    $('#content').on('click', 'a.tab-link', function(e) {
        e.preventDefault();
        $('div#dashboard_tabs').find('div[id^=dashboard]').each(
            function() {
                $(this).css('visibility', 'hidden').css(
                    'position', 'absolute');
            });
        var attr = $(this).attr('id');
        $('#' + 'dashboard-' + attr).css('visibility', 'visible').css(
            'position', 'relative');
...
function TinyMCEStart(elem, mode) {
    var plugins = [];
    if (mode == 'extreme') plugins = [
        "advlist anchor autolink autoresize autosave bbcode charmap code contextmenu directionality ",
...
function SparkLineDrawBarGraph(elem, arr, color) {
    if (color) var stacked_color = color;
    else stacked_color = '#6AA6D6';
    elem.sparkline(arr, {
        type: 'bar',
        barWidth: 7,
        highlightColor: '#000',
    ...
    $.fn.beautyTables = function() {
        ...
        this.on('keydown', function(event) {
            ...
            if (target.tagName.toUpperCase() == 'INPUT') {
                if (event.shiftKey === true) switch (event.keyCode) {
                    case 37:
                        col.prev().children(
                    ...
                    case 39:
                        ...
                        break;
                    case 40:
                        if (string_fill == false) tr.next()
                            .find('td:eq(' + col.index() +
                                ') input[type=text]').focus();
                        break;
                    case 38:
                        if (string_fill == false) tr.prev()
                            .find('td:eq(' + col.index() +
                                ') input[type=text]').focus();
                        break;
                }
                if (event.ctrlKey === true) switch (event.keyCode) {
                    case 37:
                        tr.find('td:eq(1)').find(
                            "input[type=text]").focus();
                        break;
                    case 39:
                        tr.find('td:last-child').find(
                            "input[type=text]").focus();
                        break;
                    case 40:
                        if (string_fill == false) table
                            .find(
                                'tr:last-child td:eq(' +
                                col.index() +
                                ') input[type=text]').focus();
                ...
                if (string_fill == false) {
                    ...
                    if (event.keyCode == 33) {
                        event.preventDefault();
                        table.find('tr:eq(1) td:eq(1)').find(
                            "input[type=text]").focus();
                    }
                }
            }
        ...
        table.find("input[type=text]").each(function() {
            $(this).on('blur', function(event) {
                ...
                if (table.find(
                    "input[name=string-fill]").prop(
                    "checked") == true) col.nextAll()
...
(function($) {
    $.fn.beautyHover = function() {
        ...
        table.on('mouseover', 'td', function() {
            ...
            var rows = $(this).closest('table').find('tr');
            rows.each(function() {
                $(this).find('td:eq(' + idx + ')').removeClass(
                    'beauty-hover');
            });
        });
    };
...
function Table2Json(table) {
    ...
    table.find("tr").each(function() {
        ...
        $("td", this).each(function(index) {
            if (index != 0) oneRow.push($("input", this).val());
        });
...
function FlotGraph2() {
    ...
    function updateLegend() {
        ...
        if (pos.x < axes.xaxis.min || pos.x > axes.xaxis.max || pos.y <
            axes.yaxis.min || pos.y > axes.yaxis.max) return;
        ...
        for (i = 0; i < dataset.length; ++i) {
            ...
            else y = p1[1] + (p2[1] - p1[1]) * (pos.x - p1[0]) / (p2[0] -
                p1[0]);
            legends.eq(i).text(series.label.replace(/=.*/, "= " + y.toFixed(
                2)));
        }
    }
    $("#box-two-content").bind("plothover", function(event, pos, item) {
        latestPosition = pos;
        if (!updateLegendTimeout) updateLegendTimeout = setTimeout(
            updateLegend, 50);
    });
}
...
function FlotGraph3() {
    var d1 = [];
    for (var i = 0; i <= 60; i += 1) d1.push([i, parseInt(Math.random() *
        30 - 10)]);
...
function FlotGraph4() {
    ...
    var d4 = [];
    for (var i = 0; i < 14; i += 0.1) d4.push([i, Math.sqrt(i * 10)]);
    var d5 = [];
    for (var i = 0; i < 14; i += 0.5) d5.push([i, Math.sqrt(i)]);
    var d6 = [];
    for (var i = 0; i < 14; i += 0.5 + Math.random()) d6.push([i, Math.sqrt(
        2 * i + Math.sin(i) + 5)]);
...
function MorrisChart3() {
    var decimal_data = [];
    for (var x = 0; x <= 360; x += 10) decimal_data.push({
        x: x,
        y: Math.sin(Math.PI * x / 180).toFixed(4),
        z: Math.cos(Math.PI * x / 180).toFixed(4)
    });
    Morris.Line({
...
function DrawAllCharts() {
    var chart1_data = [
        ['Smartphones', 'PC', 'Notebooks', 'Monitors', 'Routers',
            'Switches'
        ],
    ...
    drawGoogleChart(chart1_data, chart1_options, chart1_element,
        chart1_type);
    var chart2_data = [
        ['Height', 'Width'],
        ['Samsung', 74.5],
    ...
    drawGoogleChart(chart2_data, chart2_options, chart2_element,
        chart2_type);
    var chart3_data = [
        ['Age', 'Weight'],
        [8, 12],
    ...
    drawGoogleChart(chart3_data, chart3_options, chart3_element,
        chart3_type);
    var chart4_data = [
    ...
    drawGoogleChart(chart4_data, chart4_options, chart4_element,
        chart4_type);
    var chart5_data = [
        ['Country', 'Popularity'],
        ['Germany', 200],
    ...
    drawGoogleChart(chart5_data, chart5_options, chart5_element,
        chart5_type);
    var chart6_data = [
        ['Year', 'Sales', 'Expenses'],
        ['2004', 1000, 400],
    ...
    drawGoogleChart(chart6_data, chart6_options, chart6_element,
        chart6_type);
    var chart7_data = [
        ['Task', 'Hours per Day'],
        ['Work', 11],
    ...
    drawGoogleChart(chart7_data, chart7_options, chart7_element,
        chart7_type);
    var chart8_data = [
    ...
    drawGoogleChart(chart8_data, chart8_options, chart8_element,
        chart8_type);
}
...
function CoinDeskGraph() {
    ...
    $.getJSON(jsonURL, function(result) {
        ...
        $('#coindesk-xchart').resize(function() {
            ...
            graphXChartResize = setTimeout(
                DrawCoinDeskXCharts, 500);
        });
        DrawCoinDeskXCharts();
        $.each(result.bpi, function(key, val) {
        ...
        $('#coindesk-google-chart').resize(function() {
            ...
            graphGChartResize = setTimeout(
                DrawCoinDeskGoogleCharts, 500);
        });
        DrawCoinDeskGoogleCharts();
        $.each(result.bpi, function(key, val) {
            var parseDate = key;
            parseDate = parseDate.split("-");
            var newDate = parseDate[1] + "/" + parseDate[2] +
                "/" + parseDate[0];
        ...
        $('#coindesk-sparklines').resize(function() {
            ...
            graphSparklineResize = setTimeout(
                DrawCoinDeskSparkLine, 500);
        });
...
function DrawCoinDeskSparkLine() {
    $('#coindesk-sparklines').sparkline(exchange_rate, {
        height: '100%',
        width: '100%'
    });
}
...
function DrawCoinDeskGoogleCharts() {
    ...
    drawGoogleChart(google_data, google_options, google_element,
        google_type);
}
...
function SmallChangeVal(val) {
    ...
    if (parseInt(result) > 1000) return [val[0] - new_val];
    if (parseInt(result) < 0) return [val[0] + new_val];
    return [result];
}

function SparklineTestData() {
    var arr = [];
    for (var i = 1; i < 9; i++) arr.push([Math.floor(1000 * Math.random())]);
    return arr;
}
...
function RedrawKnob(elem) {
    elem.animate({
        ...
        easing: 'swing',
        progress: function() {
            $(this).val(parseInt(Math.ceil(elem.val()))).trigger(
                'change');
        }
    });
}
...
function SparklineLoop() {
    SparkLineDrawBarGraph($('#sparkline-1'), sparkline_arr_1.map(
        SmallChangeVal));
    SparkLineDrawBarGraph($('#sparkline-2'), sparkline_arr_2.map(
        SmallChangeVal), '#7BC5D3');
    SparkLineDrawBarGraph($('#sparkline-3'), sparkline_arr_3.map(
        SmallChangeVal), '#B25050');
}
...
function MorrisDashboard() {
    Morris.Line({
        ...
        xkey: 'period',
        ykeys: ['Win8', 'Win7', 'Vista', 'NT', 'XP', 'Linux', 'Mac',
            'Mobile'
        ],
        labels: ['Win8', 'Win7', 'Vista', 'NT', 'XP', 'Linux',
            'Mac', 'Mobile'
        ]
    });
    Morris.Donut({
...
function DrawSparklineDashboard() {
    ...
    $('.bar').each(function() {
        $(this).sparkline(sparkline_clients.map(SmallChangeVal), {
            type: 'bar',
            barWidth: 5,
            highlightColor: '#000',
            barSpacing: 2,
            height: 30,
            stackedBarColor: '#6AA6D6'
        });
    });
    var sparkline_table = [
        [1, 341],
...
function DrawKnobDashboard() {
    var srv_monitoring_selectors = [$("#knob-srv-1"), $("#knob-srv-2"), $(
        "#knob-srv-3"), $("#knob-srv-4"), $("#knob-srv-5"), $(
        "#knob-srv-6")];
...
function LoadTestMap() {
    $.getJSON("http://www.telize.com/geoip?callback=?", function(json) {
        ...
        var googlesattelite = new OpenLayers.Layer.Google(
            "Google Sattelite", {
                type: google.maps.MapTypeId.SATELLITE,
                numZoomLevels: 22
            });
        var map1_layers = [googlestreets, osmap, googlesattelite];
        var map1 = drawMap(json.longitude, json.latitude, "map-1",
            map1_layers);
        ...
        var sattelite = new OpenLayers.Layer.Google(
            "Google Sattelite", {
                type: google.maps.MapTypeId.SATELLITE,
                numZoomLevels: 22
            });
        var map3_layers = [sattelite];
        var map3 = drawMap(json.longitude, json.latitude, "map-3",
            map3_layers);
...
function FullScreenMap() {
    $.getJSON("http://www.telize.com/geoip?callback=?", function(json) {
        ...
        var map1_layers = [googlestreets, osmap, googlesattelite];
        var map_fs = drawMap(json.longitude, json.latitude,
            "full-map", map1_layers);
    });
}
...
function displayFlickrImages(data) {
    ...
    $.each(data.items, function(i, item) {
        ...
        res = "<a href=" + item.link + " title=" + item.title +
            " target=\"_blank\"><img alt=" + item.title + " src=" +
            item.media.m + " /></a>";
...
function FormLayoutExampleInputLength(selector) {
    var steps = ["col-sm-1", "col-sm-2", "col-sm-3", "col-sm-4", "col-sm-5",
...
function CreateAllSliders() {
    ...
    slider_range.slider({
        ...
        values: [75, 300],
        slide: function(event, ui) {
            slider_range_amount.val("$" + ui.values[0] + " - $" +
                ui.values[1]);
        }
    });
...
function DrawCalendar() {
    ...
    var calendar = $('#calendar').fullCalendar({
        ...
        select: function(start, end, allDay) {
            ...
            $('#event_submit').on('click', function() {
                var new_event_name = $('#newevent_name')
                    .val();
                if (new_event_name != '') calendar.fullCalendar(
                    'renderEvent', {
        ...
        eventRender: function(event, element, icon) {
            if (event.description != "") element.attr('title',
                event.description);
        },
        eventClick: function(calEvent, jsEvent, view) {
            var form = $('<form id="event_form">' +
                '<div class="form-group has-success has-feedback">' +
                '<label">Event name</label>' + '<div>' +
                '<input type="text" id="newevent_name" value="' +
                calEvent.title +
                '" class="form-control" placeholder="Name of event">' +
                '</div>' + '<label>Description</label>' +
                '<div>' +
                '<textarea rows="3" id="newevent_desc" class="form-control" placeholder="Description">' +
                calEvent.description + '</textarea>' +
                '</div>' + '</div>' + '</form>');
            var buttons = $(
                '<button id="event_cancel" type="cancel" class="btn btn-default btn-label-left">' +
                '<span><i class="fa fa-clock-o txt-danger"></i></span>' +
    ...
    $('#new-event-add').on('click', function(event) {
        ...
        if (event_name != '') {
            var event_template = $(
                '<div class="external-event" data-description="' +
...
$(document).ready(function() {
    ...
    if (ajax_url.length < 1) ajax_url = 'ajax/dashboard.php';
    ...
    $('.main-menu').on('click', 'a', function(e) {
        ...
        if ($(this).hasClass('dropdown-toggle') || $(this).closest(
            'li').find('ul').length == 0) {
            $(this).addClass('active-parent');
            var current = $(this).next();
            if (current.is(':visible')) {
                li.find("ul.dropdown-menu").slideUp('fast');
                li.find("ul.dropdown-menu a").removeClass(
                    'active');
            } else {
                another_items.find("ul.dropdown-menu").slideUp(
                    'fast');
                current.slideDown('fast');
            }
        } else if (li.find('a.dropdown-toggle').hasClass(
            'active-parent')) {
            var pre = $(this).closest('ul.dropdown-menu');
            pre.find("li.dropdown").not($(this).closest('li')).find(
                'ul.dropdown-menu').slideUp('fast');
        }
        if ($(this).hasClass('active') == false) {
            $(this).parents("ul.dropdown-menu").find('a').removeClass(
                'active');
            $(this).addClass('active');
        }
        if ($(this).hasClass('ajax-link')) {
            e.preventDefault();
            if ($(this).hasClass('add-full')) $('#content').addClass(
                'full-content');
            else $('#content').removeClass('full-content');
    ...
    $('#main').css('min-height', height).on('click', '.expand-link',
        function(e) {
            ...
            button.toggleClass('fa-expand').toggleClass(
                'fa-compress');
    ...
    $('#screen_unlock').on('mouseover', function() {
        ...
        var form = $(
            '<div class="form-group"><label class="control-label">Username</label><input type="text" class="form-control" name="username" /></div>' +
            '<div class="form-group"><label class="control-label">Password</label><input type="password" class="form-control" name="password" /></div>'
        );
        var button = $(
            '<div class="text-center"><a href="index.html" class="btn btn-primary">Unlock</a></div>'
        );
