...
class DashboardsController extends AppController {
    ...
    public function index() {
        ...
        $compCount = $this->Computer->query("SELECT count(*) FROM computers"
                . " JOIN usersec ON usersec.computerid = computers.computerid"
                . " JOIN users ON users.userid = usersec.userid"
                . " WHERE users.name = '".$_SESSION['Username']."'");
        $this->set('compCount', $compCount);

        $clientCount = $this->Client->query("SELECT count(Clients.clientid) FROM Clients "
                . "JOIN usersec ON usersec.clientid = Clients.clientid "
                . "JOIN users ON users.userid = usersec.userid AND usersec.computerid = '0' "
                . "WHERE users.name = '".$_SESSION['Username']."'");
        $this->set('clientCount', $clientCount);

        $offlineCount = $this->Computer->query("SELECT COUNT(computers.computerid) as count FROM computers
                 WHERE (os LIKE '%server%' OR os LIKE '%linux%') AND
 ((computerid IN (
 SELECT heartbeatcomputers.computerid 
 FROM heartbeatcomputers
 ...
 WHERE users.name = '" . $_SESSION['Username'] . "' AND LastHeartbeatTime <  DATE_ADD(NOW(),INTERVAL -(SELECT heartbeatdbupdateinterval *3 FROM advancedconfig)SECOND))))");
        ...
        $ticketCount = $this->Ticket->query(""
                . "SELECT count(*) "
                . "FROM TICKETS "
                . " WHERE `status`='1' OR `status`='2' AND externalid != NULL");
        $this->set('ticketCount', $ticketCount);

        $clientDrop = $this->Client->find('list', 
                array('fields' => array('Client.ClientID', 'Client.Name'),
                                'joins' => array(
                array(
                    'table' => 'usersec',
                    'alias' => 'usersec',
    ...
    public function view($clientid) {
        if (!($client = $this->Client->find('first', array(           
            ...
            'conditions' => array('Client.ClientID' => $clientid, 'Users.name' => $_SESSION['Username'])
                )))) {
