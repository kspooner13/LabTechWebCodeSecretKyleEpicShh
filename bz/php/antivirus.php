   ...
   $field="`Antivirus Details`";
    ...
    echo "<p><hr width=70%><p><h2><b>Antivirus Health Score Details{$exclusions_text}</b></h2>
	  <table class=TBscore>
	    <tr align=left>
		  ...
		  <th class=event_date width=150><a href=\"../php/antivirus.php?computerid={$computerid}&sorting=".$sort."&field=Event Date\">Event Date</a></th>
		  <th class=score_detail width=450><a href=\"../php/antivirus.php?computerid={$computerid}&sorting=".$sort."&field=Antivirus Details\">Antivirus Details</a></th>
		 </tr>";
    ...
    while ($row = mysqli_fetch_array($result)) {
		...
		<td class=names><a href=\"labtech:open?computerid=".$row["computerid"]."\">".$row['Status']."&nbsp;&nbsp;".$row["Computer Name"]."</a></td>
			...
			<td class=score_detail style='text-align:left;'>".$row["Antivirus Details"]."</td>
		</tr>";
    }
   echo "</table>";
