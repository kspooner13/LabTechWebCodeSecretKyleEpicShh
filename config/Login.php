    ...
    private function dologinWithPostData()
    {
        ...
        if (empty($_POST['username'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                // username field)
                $sql = "SELECT `name`, email, last_date, AES_DECRYPT(password, UserID) as user_password_hash
                        FROM users
                ...
                if ($result_of_login_check->num_rows == 1) {
                    ...
                    if ($_POST['password'] == $result_row->user_password_hash) {

                        // write user data into PHP SESSION (a file on your server)
                        $_SESSION['user_name'] = $result_row->name;
                        $_SESSION['user_email'] = $result_row->email;
						$_SESSION['last_login'] = $result_row->last_date;
