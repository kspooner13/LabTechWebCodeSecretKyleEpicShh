<!DOCTYPE html>
<html>
<head><title>Marco Managed Health Scores | Antivirus Details</title>
...
elseif($_GET["field"]=="`Antivirus Details`")
{
   $field="`Antivirus Details`";
}


	$sql = "SELECT computerid, computers.name as 'Computer Name', eventdate as 'Event Date', SUBSTRING(stat15,8) as 'Antivirus Details' FROM h_extrastatsdaily JOIN computers USING (computerid) WHERE computers.computerid={$computerid} AND INSTR(stat15,\";.,\") AND eventdate > DATE_ADD(NOW(), INTERVAL -1 MONTH) ORDER BY {$field} {$sort}";

	$result = mysql_query($sql);

    echo "<p><hr width=70%><p><h2><b>Antivirus Health Score Details{$exclusions_text}</b></h2>
	  <table class=TBscore>
	    <tr align=left>
		  <th width=200 class=names>Computer Name</th>
		  <th class=event_date width=150><a href=\"antivirus.php?computerid={$computerid}&sorting=".$sort."&field=Event Date\">Event Date</a></th>
		  <th class=score_detail width=450><a href=\"antivirus.php?computerid={$computerid}&sorting=".$sort."&field=Antivirus Details\">Antivirus Details</a></th>
		 </tr>";
    ...
    while ($row = mysql_fetch_array($result)) {
		...
		<td class=names><a href=\"labtech:open?computerid=".$row["computerid"]."\">".$row["Computer Name"]."</a></td>
			...
			<td class=score_detail>".$row["Antivirus Details"]."</td>
		</tr>";
    }
   echo "</table>";
