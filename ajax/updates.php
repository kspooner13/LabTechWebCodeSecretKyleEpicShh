...
<div id="space">
	<div id="section-two">
	 <?php
		...
		$sql = "SELECT computerid, computers.name as 'Computer Name', eventdate as 'Event Date', SUBSTRING(stat20,7) as 'Updates Details' FROM h_extrastatsdaily JOIN computers USING (computerid) WHERE computers.computerid={$computerid} AND INSTR(stat20,\";^\") AND eventdate > DATE_ADD(NOW(), INTERVAL -1 MONTH) ORDER BY {$field} {$sort}";
		...
		echo "<p><hr width=70%><p><h2><b>Updates Health Score Details{$exclusions_text}</b></h2>
		  <table class=TBscore>
			<tr align=left>
			  ...
			  <th class=event_date width=150><a href=\"updates.php?computerid={$computerid}&sorting=".$sort."&field=Event Date\">Event Date</a></th>
			  <th class=score_detail width=500><a href=\"updates.php?computerid={$computerid}&sorting=".$sort."&field=Updates Details\">Updates Details</a></th>
			 </tr>";
	  ...
	  <p><br><h3><strong>How this score is calculated:</strong></h3>
		<table class=score_explain>
		  <tr align=left>
			<td><ul style="list-style-type:disc"><li>Detects if there are any outstanding Standard Patches that are not installed. Standard patches are defined as any patch not categorized by Microsoft as a 'Critical' or 'Security' patch, and does not have any severity assigned to it.</li>
