   ...
   $field="`Disk Details`";
    ...
    echo "<p><hr width=70%><p><h2><b>Disk Health Score Details{$exclusions_text}</b></h2>
	  <table class=TBscore>
	    <tr align=left>
		  ...
		  <th class=event_date width=150><a href=\"disk.php?computerid={$computerid}&sorting=".$sort."&field=Event Date\">Event Date</a></th>
		  <th class=score_detail width=450><a href=\"disk.php?computerid={$computerid}&sorting=".$sort."&field=Disk Details\">Disk Details</a></th>
		 </tr>";
    ...
    while ($row = mysql_fetch_array($result)) {
		...
		<td class=names><a href=\"labtech:open?computerid=".$row["computerid"]."\">".$row["Computer Name"]."</a></td>
			...
			<td class=score_detail>".$row["Disk Details"]."</td>
		</tr>";
    }
   echo "</table>";
...
<table class=score_explain>
  <tr align=left>
    <td>This check is unique in that the check is calculated against each internal hard drive. In other words, if there are three internal hard drives identified, and two of them pass a check weighted at 50%, the check is worth 67% (100%, 100% + 1%)/3) the weight value for that check, which is 33%.</td></tr></table>
</p>
<br></div></div>
<div id="footer">
