...
$sort="DESC";
	...
	$sql = "SELECT computerid, computers.name as 'Computer Name', eventdate as 'Event Date', SUBSTRING(stat14,6) as 'Events Details' FROM h_extrastatsdaily JOIN computers USING (computerid) WHERE computers.computerid={$computerid} AND INSTR(stat14,\";\") AND event14 <> -1 AND eventdate > DATE_ADD(NOW(), INTERVAL -1 MONTH) ORDER BY {$field} {$sort}";
    ...
    echo "<p><hr width=70%><p><h2><b>Events Health Score Details{$exclusions_text}</b></h2>
	  ...
	  <table class=TBscore>
	    <tr align=left>
		  ...
		  <th class=event_date width=150><a href=\"events.php?computerid={$computerid}&sorting=".$sort."&field=Event Date\">Event Date</a></th>
		  <th class=score_detail width=500><a href=\"events.php?computerid={$computerid}&sorting=".$sort."&field=Events Details\">Events Details</a></th>
		 </tr>";
    ...
    while ($row = mysql_fetch_array($result)) {
		...
		<td class=names><a href=\"labtech:open?computerid=".$row["computerid"]."\">".$row["Computer Name"]."</a></td>
			...
			<td class=score_detail>".$row["Events Details"]."</td>
		</tr>";
    }
   echo "</table>";
...
<table class=score_explain>
  <tr align=left>
    <td><ul style="list-style-type:disc"><li>Detects if there are any critical errors from the event logs recorded by scripts.</li>
