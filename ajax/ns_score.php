<!DOCTYPE html>
<html>
<head><title>Marco Managed Health Scores | Network Specialist</title>
...
FROM hc_scores JOIN v_extradataclients USING (clientid) WHERE v_extradataclients.`Ignore - Non Contractual Services` <> 1 AND `Network Specialist` = '{$net_sp}' GROUP BY `Client Name` ORDER BY {$field} {$sort}";
	} else {
	$sql = "SELECT 	`Client Name`, 
...
FROM hc_scores JOIN v_extradataclients USING (clientid) WHERE `Network Specialist` = '{$net_sp}' GROUP BY `Client Name` ORDER BY {$field} {$sort}";
	}
	$result = mysql_query($sql);

if ($exclusions == "true") {
    echo "<p><hr align=center width=75%><p><h2><b>{$net_sp} Team Health Scores{$exclusions_text}</b></h2>
	<table class=TBscore>
	  <tr>
	    <th class=names width=300><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Client Name\">Client Name</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Antivirus\">Antivirus</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Disk\">Disk</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Intrusion\">Intrusion</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Usability\">Usability</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Services\">Services</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Updates\">Updates</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Events\">Events</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&exclusions=true&sorting=".$sort."&field=Overall Score\">Overall Score</a></th>
	  </tr>";
	  } else {
	echo "<p><hr align=center width=75%><p><h2><b>{$net_sp} Team Health Scores{$exclusions_text}</b></h2>
	<table class=TBscore>
	  <tr>
	    <th class=names width=300><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Client Name\">Client Name</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Antivirus\">Antivirus</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Disk\">Disk</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Intrusion\">Intrusion</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Usability\">Usability</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Services\">Services</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Updates\">Updates</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Events\">Events</a></th>
		<th class=stats><a href=\"ns_score.php?net_sp={$net_sp}&sorting=".$sort."&field=Overall Score\">Overall Score</a></th>
	  </tr>";
	  }
...
?>
	  <p><br><h3><strong>Disclaimer:</strong></h3>
		<table class=score_explain>
		  <tr align=left>
			<td><ul style="list-style-type:disc"><li>Health scores are calculated and updated daily at 7:00AM Central Time.</li>
					<li>These scores may not reflect all clients or all exclusions.</li>
					<li>These scores are intended to be a general idea of where your customers current health is and for further discovery and visibility into poor scores.  You will be able to dive into individual clients and scores to discover where focused efforts can be directed for improvements.</li>
				</ul>
			</td>
		  </tr>
		</table>
