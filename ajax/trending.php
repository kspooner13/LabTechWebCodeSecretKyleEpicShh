...
elseif($_GET["field"]=="Percent Change")
{
   $field="`Percent Change`";
	...
	$sql = "SELECT x.`Client Name`, x.`CARE Team`,IF(`Managed IT - Active` = 1, 'Yes','No') 'Active Customer', CONCAT(LEFT(y.checkdate,10)) 'Previous Date', CONCAT(LEFT(x.checkdate,10)) 'Latest Date', y.`{$trend}` 'Previous Score', x.`{$trend}` 'Latest Score', ROUND((((x.`{$trend}` - y.`{$trend}`)/100)*100),2) 'Percent Change'
  ...
  JOIN hc_scores_weekly Y ON (y.clientid=x.clientid) JOIN v_extradataclients ON x.clientid=v_extradataclients.clientid
    ...
    echo "<p><hr align=center width=75%><p><h2><b>{$trend} Trending</b></h2>
	  <table class=TBscore>
	    <tr>
		  <th width=300 class=names><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=Client Name\">Client Name</a></th>
		  <th class=stats width=125><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=CARE Team\">CARE Team</a></th>
		  <th class=stats width=150><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=Active Customer\">Active Customer</a></th>
		  <th class=stats width=150><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=Previous Date\">Previous Date</a></th>
		  <th class=stats width=150><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=Latest Date\">Latest Date</a></th>
		  <th class=stats><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=Previous Score\">Previous Score</a></th>
		  <th class=stats><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=Latest Score\">Latest Score</a></th>
		  <th class=stats><a href=\"trending.php?trend={$trend}&sorting=".$sort."&field=Percent Change\">Percent Change</a></th>
		 </tr>";
	...
	while($row = mysql_fetch_array($result)) {
        echo "<tr align=center>
			...
			<td class=stats>".$row["Active Customer"]."</td>
